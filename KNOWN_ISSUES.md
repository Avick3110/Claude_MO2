...
Current as of v2.9.3. These are known limitations, not bugs. For the full version history see `mo2_mcp/CHANGELOG.md`.
...
- **Multi-condition record types beyond QUST top-level (deferred to v2.9.x).** Mutagen 0.53.1 exposes nested `*Conditions` properties on `IQuestAliasGetter` and `IQuestLogEntryGetter` (alias-level + log-entry-level conditions on quest sub-records). `add_conditions`/`remove_conditions` only address top-level major-record condition lists today; reaching nested sub-records requires extension of v2.9.1's `condition_target` mapping table to address nested-major sub-records, similar to v2.9.0's INFO override pattern (parent-topic-resolution + child-DeepCopy). Phase 1's race-probe schema sweep flagged both surfaces; first v2.9.x consumer trigger lands the extension.
...
- **QUST.Aliases / Stages / Objectives.** Out of scope for the current Effects-list mechanism even though the schema shape is similar — sub-class polymorphism makes them harder, and no real consumer has surfaced yet. PERK.Effects (the heavier half of this carry-over) closed as of v2.9.3 — see § Covered as of v2.9.3 below.

### Covered as of v2.9.3

- **PERK.Effects writability (v2.9.3).** `set_fields: {Effects: [{type: <subclass>, ...}]}` on PERK records accepts every concrete `APerkEffect` leaf per Phase 1's audit (`APERK_EFFECTS_AUDIT.md`): `PerkAbilityEffect`, `PerkEntryPointAbsoluteValue`, `PerkEntryPointAddActivateChoice`, `PerkEntryPointAddLeveledItem`, `PerkEntryPointAddRangeToValue`, `PerkEntryPointModifyActorValue`, `PerkEntryPointModifyValue`, `PerkEntryPointModifyValues`, `PerkEntryPointSelectSpell`, `PerkEntryPointSelectText`, `PerkEntryPointSetText`, `PerkQuestEffect`. Replace-semantics on the Effects array, mirroring v2.8.0's posture for SPEL/ALCH/ENCH/SCRL/INGR. Branch A in `ConvertJsonElementToListItem` extends with `typeof(APerkEffect)` routing to a new `BuildPerkEffectFromJson` factory (next to `BuildConditionFromJson`) that reflects `Mutagen.Bethesda.Skyrim.{TypeName}`, rejects abstract types + non-`APerkEffect`-assignable types, then walks each non-discriminator JSON member through `SetPropertyByPath`. Carrier dispatch is property-type-driven via reflection (`Perk.Effects` exposes `ExtendedList<APerkEffect>`), so adding the type-special-case automatically opens PERK with no carrier-list edit needed. Per-effect Conditions on the `APerkEffect` base use a TWO-LEVEL nesting: outer `Conditions` is a list of `PerkCondition` wrappers each carrying `RunOnTabIndex` (int) plus an inner `Conditions` list whose entries take the same shape as the `add_conditions` operator. **Q4 lock (v2.9.0 dispatcher composes UNTOUCHED) verified end-to-end** via Phase 2's composition probe: PEPM Effect with nested `HasPerk` parameters round-tripped via bridge subprocess; readback walked Branch A → `BuildPerkEffectFromJson` → `SetPropertyByPath` (outer Conditions) → Branch A (PerkCondition wrapper) → `SetPropertyByPath` (inner Conditions) → Branch A (`typeof(Condition)`) → `BuildConditionFromJson` → `BuildCondition` → v2.9.0 `RouteParameterSlot` → `IFormLinkOrIndex<IPerkGetter>` and resolved the supplied perk FormLink. `PerkQuestEffect.Unknown` is an opaque `MemorySlice<Byte>` blob and is rejected at write-time with a clean error — omit it; the source record's value carries through. **Real-consumer signal:** Phase 3 verified end-to-end against Authoria's Requiem-Reforged modlist by rebalancing the Requiem-overridden "Electromancy" perk (`Skyrim.esm:10FCFA` winning at load order 1187 in `Requiem - Magic Redone.esp`) from 1.2× to 1.1× ModSpellMagnitude — Requiem's renamed Description, lowered-threshold (60→50) top-level Conditions, and all sibling fields (Trait/Level/NumRanks/Playable/Hidden/EditorID) preserved byte-identical while the Effects array was fully replaced. Heterogeneous multi-leaf array writes confirmed against Dawnguard.esm's PlayerWerewolfFeed (3 distinct concrete leaves in one Effects array, including VMAD sibling preservation). **Coverage notes:** 4 of the 12 leaves (`PerkEntryPointAbsoluteValue`, `PerkEntryPointAddLeveledItem`, `PerkEntryPointAddRangeToValue`, `PerkEntryPointModifyValues`) have zero vanilla + 4-DLC effect instances per Phase 1's frequency probe over 523 PERK records; coverage-smoke uses synthetic round-trip evidence for them. Modder-authored use surfaces if/when a consumer hits one.
- **TranslatedString single-language convenience plumbing (v2.9.3 bridge).** A JSON String fed to a slot typed `Mutagen.Bethesda.Strings.TranslatedString` now writes as an English-language entry (via a one-branch addition to `ConvertJsonValue` mirroring the v2.8.0 IFormLinkNullable single-field FormLink pattern directly above). Required for `PerkEntryPointSetText.Text` to satisfy Q2 = A's "ship all 12 leaves" promise; surface expansion is inert (no other operator advertises TranslatedString slot writes outside the v2.9.3 PEPSetText case).

### Covered as of v2.9.2

- **Read-side efficiency for `mo2_record_detail` (v2.9.2).** Three composable optional parameters on the existing tool: `formids: [...]` (batch read — N records per subprocess invocation, amortizes the ~889 ms median startup), `fields: [...]` (projection — dot-segmented paths with auto-traversal of lists and dicts mid-path; out-of-projection branches omitted), `expand_links: [...]` (single-level FormLink expansion — wrapper form `{formid, EditorID, expanded: {...}}` at named positions; no recursion). All three composable on a single call and orthogonal to `resolve_links`. Per Q6 lock (2026-04-28), `formids` × `plugin_names` returns the cross-product (N×M cells, each with its own success/error envelope) — tested cliff-free up to N×M=1000 at 11.7 s wall-clock. Validation is strict-batch: bad field paths and bad expansion targets accumulate per record-type into one error response per § D's multi-error contract. Per-record formid resolution failures are partial (top-level `success: true`; per-record envelope per Q3 lock matching the existing `read_records` precedent). Defaults (all three parameters absent) preserve v2.9.1 single-record / full-payload / no-expansion behavior bit-identically.
- **Cross-master FormLink expansion (v2.9.2 Phase 4).** Phase 3 surfaced bug B5 live on Authoria: `expand_links` against a record whose winning plugin is a mod ESP (e.g. `Authoria - Requiem Master Patch.esp`) whose FormLinks point back into Skyrim.esm-originated records returned the missing-master error envelope. Phase 4's Option B fix: new optional `available_plugins` parameter on `ReadRequest` + `ReadBatchRequest`; the Python wrapper passes the full enabled load-order plugin path list (via the new `_build_available_plugins(idx)` helper) whenever `expand_links` is supplied; the bridge's `ExpandFormLinkValue` walker lazy-hot-loads the matching plugin on the first miss for that originating-master filename, caches it in `modCache`, retries the lookup. Lazy: zero cost when in-master resolves, ~one Mutagen plugin load per cross-master target master per batch. Architecturally-correct foundation for future override-aware expansion (a v2.9.x candidate — the lookup-logic swap is the only remaining work). Coverage-smoke `1.P.expand.crossmaster` cell + race-probe synthetic two-plugin fixture exercise the post-fix path; the existing `4.dsl.06` cell flipped from SKIP-with-reason to PASS via a synthetic missing-master fixture (target master deliberately absent from `available_plugins`) — verifies the Q2 uniform null-safety wrapper-form contract end-to-end.

### Read-surface candidates (v2.9.x)

These are read-side capabilities not addressed in v2.9.2 — Aaron's xEdit-clarity vision ("the big picture goal is not just for claude to have the ability to make and edit anything accurately but also to see everything as clearly as a user can in xedit") motivates surfacing them as candidates. Real-consumer signal drives sequencing.

- **Reverse-link search.** Given a FormID, find all records that reference it. Direction is FORMID → records-that-reference-it (opposite of v2.9.2's `expand_links`, which goes record → FormLinks-it-contains). Highest viewing-assistant impact: directly answers "what other mods reference this MGEF / perk / faction." Needs an inverted-index data structure (indexed at record-build time or lazily on first call). New mechanism / new tool surface; not an incremental extension of `mo2_record_detail`. v2.9.x candidate.
- **Override-aware FormLink expansion.** `ExpandFormLinkValue` today returns the originating-master version of the linked record (via filename-match against the `modCache`). xEdit shows the load-order winner — the override that survives all the patches. v2.9.2 Phase 4 lays the foundation by loading the full load order on demand into `modCache`; the lookup-logic swap (use Mutagen's `LinkCache.TryResolve` to find the winning override across all cached plugins, instead of the first filename match) is the remaining v2.9.x work. Adds an `expand_winner: bool` parameter or flips the default once Aaron decides which is the right behavior on Authoria.
- **`MaxDepth` MCP-configurable.** Currently hardcoded to 6 in `ReadRequest.MaxDepth` (and `ReadBatchRequest.MaxDepth`). v2.9.x candidate to expose if a real consumer hits the depth limit on projected/expanded reads (projection narrows the walker so depth-limit hits are less likely than on full-payload reads, but the limit itself is unchanged).
- **Cross-call result caching.** The bridge subprocess is per-call — every `mo2_record_detail` invocation pays the ~889 ms startup cost and discards modCache state on exit. Cross-call state (e.g. an MCP-server-side cache keyed by `(plugin_path, mtime)` that survives across calls and warm-starts the Mutagen overlay) is a different workstream. Real-consumer-driven if it surfaces.

### Covered as of v2.9.1

- **Quest condition disambiguation (v2.9.1).** QUST records carry `DialogConditions` and `EventConditions` rather than a single `Conditions` list. The new `condition_target` operator parameter on `add_conditions`/`remove_conditions` selects which list to write to / remove from. Required on QUST; missing `condition_target` on a QUST `add_conditions`/`remove_conditions` call surfaces a clean per-record error naming the available targets (`'dialog'` → `DialogConditions`, `'event'` → `EventConditions`; case-insensitive). Records with a single `Conditions` property (MagicEffect/Perk/Package/IdleAnimation/INFO/etc.) reject `condition_target` explicitly. Records with no `Conditions` list at all (e.g. ARMO) preserve the existing Tier D `unmatched_operators` shape bit-identically. Phase 1 schema probe confirmed Quest is the sole multi-condition top-level record type in Mutagen 0.53.1; nested-conditions surfaces (`IQuestAliasGetter.Conditions` + `IQuestLogEntryGetter.Conditions`) are deferred to v2.9.x — see entry above.
...
- **Record index can stay cache-stale after rm + F5 of a previously-written test patch.** Surfaced repeatedly during patch-iteration workflows (Phase 3 + Phase 5 of v2.9.1 ship hit it twice). After `rm`'ing a test ESP and pressing F5, the next `mo2_create_patch` against a FormID the deleted ESP overrode can fail with `"Plugin file not found: <deleted-ESP>"` — the index still has the deleted ESP cached as the winning source for that record. **Workaround:** call `mo2_build_record_index(force_rebuild=true)` after the F5 (~76 s cold rebuild on ~3000+ plugin modlists; client-side timeout per the entry below — server completes regardless). Pre-existing MO2/MCP cache-hygiene behavior, not a v2.9.x regression. v2.9.x DX candidate to detect and self-heal automatically.

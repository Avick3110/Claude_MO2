...
| `4.dsl.06` | **Phase 4 absorbed.** `mo2_record_detail` against a synthetic two-plugin fixture (built via Mutagen `WriteToBinary` at coverage-smoke / race-probe runtime): orphan override plugin's RACE.ActorEffect references a SPEL whose originating master `GhostMaster.esm` is never written to disk; `available_plugins` supplied but does NOT include `GhostMaster.esm` | response's `ActorEffect` list renders as wrapper-form objects per `1.P.expand.RACE.list`, with the missing-master entry rendered uniform-shape per Q2 lock: `{formid: "GhostMaster.esm:000900", EditorID: null, expanded: null, error: "FormID target not in load order"}`. Top-level `success: true`; the bridge does NOT fail the whole call. Phase 4 absorption flipped this cell from SKIP-with-reason to PASS — synthetic fixture lives in `Test 424` body and is cleaned up at section end |
| `1.P.expand.crossmaster` | **Phase 4 added (Test 425).** `read_record(plugin_path=<override>, formid=<override:RACE>, expand_links=["ActorEffect"], available_plugins=[<master>, <override>])` against a synthetic two-plugin fixture where the override RACE's ActorEffect references a SPEL whose originating master is the supplied master plugin | response resolves the cross-master expansion: wrapper carries no error key, `EditorID` matches the master's SPEL EditorID, `expanded` is an object dict carrying the master record's full RecordReader walk. Verifies the v2.9.2 P4 Option B fix end-to-end via bridge subprocess. Distinct from the in-master cells (`1.P.expand.RACE.list`) which only exercise FormLink-targets-in-same-plugin |
...
|---|---|---|
| `5.range` | `dev/plans/v2.9.1_quest_condition_disambiguation/MATRIX.md` Layer 1.P + 1.D + 2 + 4 + 5 (**400 v2.9.1 cells confirmed via coverage-smoke** — 382 v2.9.0 + 18 v2.9.1, 6 SKIP-with-reason) | each cell PASS as it did in v2.9.1 P5; Phase 2 added 24 new cells (Tests 401–424; 1 SKIP at 4.dsl.06); Phase 4 flipped 4.dsl.06 SKIP→PASS via synthetic fixture and added cross-master positive cell `1.P.expand.crossmaster` (Test 425). **Post-Phase-4 total = 425 cells, 6 SKIP-with-reason** |
...
Phase 2 closes with these MATRIX edits landed (all completed 2026-04-28):

- [x] **Layer 5 cell count confirmed** — pre-v2.9.2 baseline confirmed at 400 cells from v2.9.1 P4 handoff (382 v2.9.0 + 18 v2.9.1, 6 SKIP-with-reason). Phase 2 adds 24 new cells (Tests 401–424; one SKIP at 4.dsl.06 for missing-master synthetic fixture). New total: **424 cells** (382 v2.9.0 + 18 v2.9.1 + 24 v2.9.2; 7 SKIP-with-reason total). Layer 5 row updated below.
- [x] **Q1–Q6 expectation flips audit** — all six locks held per Phase 1 hand-back; no further matrix flips needed. Q6 amendment items already landed in Phase 1 (`1.D.07` removed, `2.05` added). Q1 auto-traversal verified at walk time: `Voices.Male` flattens the gendered-IEnumerable to a list of FormID strings (Mutagen 0.53.1 exposes `IGenderedItemGetter` as `IEnumerable<T>` yielding both Male+Female in iteration order — Phase 2 walker matches v2.9.1 RenderValue's IEnumerable handling). Q4 pre-flight: validation runs against `IRaceGetter`'s reflected property set (interface inheritance walked to include inherited `EditorID`/`FormKey` from `IMajorRecordGetter`); fail-fast before any walk happens; rollback contract holds.
- [x] **Layer 2.04 single-record path response shape locked** — single-`formid` + new params returns the v2.9.1 single-record shape (top-level `formid`/`record_type`/`editor_id`/`fields` keys), NOT the per-record envelope shape (which is reserved for `formids: [...]` batch + `plugin_names` paths). Verified by Test 417 cell. § A's "Composes with `fields`/`expand_links`" lock confirmed.
- [x] **Layer 2.05 cross-product JSON envelope shape locked** — flat list `{success, records: [{formid, plugin_name, success, ...}, ...]}` matching the existing `plugin_names` shape extended with the `formid` key. Each cell carries: `formid`, `plugin_name` (the requested plugin), `editor_id` (resolved), `load_order` (from index), `record_type`, `success`, `fields` (on success) or `error` (on failure). Top-level `success: true` if any cell succeeded; `success: false` only if every cell failed. Documented in schema description for `formids` parameter; see `_handle_formids_batch` in `tools_records.py` and Phase 2 Smoke path (e).
- [x] **Error message wording finalized** — Phase 2 locked these exact strings (transcribed from `RecordReader.cs` + `tools_records.py`):
  - Empty list rejections: `"formids must contain at least one FormID — empty list rejected. Omit the parameter for single-record mode."`, `"fields must contain at least one path — empty list rejected. Omit the parameter for full-payload mode."`, `"expand_links must contain at least one path — empty list rejected. Omit the parameter for no-expansion mode."`. Symmetric across Python wrapper + bridge.
  - Validation-failure top-level: `"Field path / expansion target validation failed."` (bridge); per-type detail under `validation_errors.<TypeCode>` with `bad_field_paths` / `bad_expansion_targets` / `non_formlink_expansion_targets` / `valid_field_names` / `valid_formlink_field_names`.
  - Missing-master expansion target: `"FormID target not in load order"` (bridge); rendered uniform-shape `{formid, EditorID: null, expanded: null, error: "..."}`.
  - Mutual-exclusion: `"Provide either formids (batch) or formid/editor_id/plugin_name (single-record), not both."` (Python wrapper).
- [x] **Layer 1.D validation-error JSON shape locked** — exact key names per the new `ValidationErrorDetail` model in `Models.cs`: `bad_field_paths`, `bad_expansion_targets`, `non_formlink_expansion_targets` (three category lists, each strings), plus `valid_field_names` and `valid_formlink_field_names` (context lists for caller fixup). Rows 1.D.01–1.D.05 unchanged from Phase 0 lock — keys match the structural contract.
- [x] **4.dsl.05 always-null carrier** — Phase 2 picked `INpcGetter.DeathItem` (most NPCs don't carry an explicit death-item drop). Test 423 verifies the projection-shape contract: when `fields=["DeathItem"]` is supplied, the response contains DeathItem (or omits it for null-on-source per the v2.9.1 walker's null-property-drop default), with no out-of-projection fields. Either rendering is acceptable; the contract is "field path resolved + no other top-level fields."
- [x] **4.dsl.06 missing-master synthetic fixture** — Phase 4 absorbed (post-Phase-3 conductor decision 2026-04-28). Synthetic fixture built via Mutagen `WriteToBinary` at coverage-smoke runtime: orphan override plugin's RACE.ActorEffect references a SPEL whose originating master `GhostMaster.esm` is never written to disk; `available_plugins` supplied without it, so the bridge's lazy hot-load can't resolve. Cell flipped from SKIP-with-reason to PASS within the existing 424-cell count. Post-Phase-4 total: **425 cells, 6 SKIP-with-reason** (4.dsl.06 dropped from SKIPs).

## Phase fill-in checklist (Phase 4 hand-back)

Phase 4 closes with these MATRIX edits landed (all completed 2026-04-28):

- [x] **4.dsl.06 absorbed** — flipped SKIP→PASS via synthetic missing-master fixture; row updated above.
- [x] **New cross-master positive cell** — `1.P.expand.crossmaster` (Test 425) added to Layer 1.P; row appended after `4.dsl.06`.
- [x] **Layer 5 cell count updated** — 424 → 425 (one new positive cell); SKIP count 7 → 6 (4.dsl.06 dropped).
- [x] **Cross-master FormLink expansion** — Phase 3's bug B5 surfaced live; Phase 4's Option B fix lands the bridge `ExpandFormLinkValue` lazy hot-load via the new `available_plugins` request parameter (Models.cs `AvailablePlugins` on both `ReadRequest` + `ReadBatchRequest`); wrapper passes the index's `_load_order` plugin paths via the new `_build_available_plugins(idx)` helper at three forwarding sites in `tools_records.py`. KNOWN_ISSUES.md updated: cross-master FormLink expansion moved to "covered as of v2.9.2"; the four v2.9.x read-surface candidates (reverse-link search, override-aware expansion, MaxDepth MCP-configurable, cross-call result caching) documented under a new "Read-surface candidates (v2.9.x)" subsection.

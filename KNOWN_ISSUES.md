...
## Condition-parameter coverage (v2.9.0 P2A + P2B + P2C)

v2.9.0 wires the **generic Condition-function parameter dispatch surface** for 188 functions across five slot-shape branches:
...
- **`Int32` and `Single` primitive branches (P2C)** — direct `JsonElement.GetInt32()` / `GetSingle()` conversion. `Int32` is the reused branch across 10 multi-slot functions' secondary slots (Stage / *AliasIndex / SceneActionIndex / Unknown's ParameterOne+Two) and pre-positions for 2D's 11 PrimitiveOnly functions (all of which use Int32). `Single` is exercised by exactly one in-scope function (GetWithinDistance.Distance).
...
These are **2D scope** within the v2.9.x release line:

- **11 primitive-only Condition functions** (Phase 2D). All use Int32 slots — `GetIsAliasRef` (ReferenceAliasIndex), `GetInCurrentLocAlias`/`GetIsEditorLocAlias`/`GetLocationAliasCleared`/`IsLocAliasLoaded` (LocationAliasIndex), `GetNumericPackageData`/`GetWithinPackageLocation`/`IsNullPackageData` (PackageDataIndex), `GetPlayerControlsDisabled` (×2 Int32), `GetVATSValueUnknown` (×2 Int32), `IsLimbGone` (Limb). Int32 branch already in place from P2C; 2D is pure `KnownParameterizedFunctions` extension + cells. Single + Boolean primitives have zero in-scope consumers in 2D so they don't change scope.

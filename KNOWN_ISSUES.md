...
- **INFO record `op: "override"` — not yet wired.** `mo2_create_patch` with `op: "override"` against an INFO FormID surfaces `"Could not create override for INFO"` per record (clean error after Phase 4's line-180 DX bonus-catch). Mutagen 0.53.1 doesn't expose a top-level `SkyrimMod.DialogResponses` group; INFO records are nested under `DialogTopic.Responses` as a plain `Noggog.ExtendedList<DialogResponses>` with no override-add helper. Real INFO override requires parent-topic `GetOrAddAsOverride` + child-response find-by-FormKey, plus rollback-semantics design (whether to remove the child response only or the entire parent topic override on partial failure) — multi-hour scope expansion deferred from Phase 4 to a Phase 4-INFO sub-session per Aaron's call (2026-04-27). v2.9.0 ships without INFO override; existing INFO read-path (`mo2_record_detail`, `mo2_query_records`, conflict analysis) is unaffected. Architectural archaeology preserved in `tools/race-probe/Program.cs` (v2.9 P4 section); reproduction trivially via `mo2_create_patch` against any INFO FormID.

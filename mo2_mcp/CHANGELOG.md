...
- **`add_conditions actor_value` parameter** (Phase 4 item 5). Condition functions whose underlying Mutagen `ConditionData` carries an `ActorValue` enum slot (`GetActorValue`, `GetBaseActorValue`, `GetActorValuePercent`, etc.) now accept an `actor_value` field on each condition entry. The bridge parses the string via `Enum.Parse<ActorValue>` and assigns it to the constructed ConditionData. Pre-v2.8.0 the underlying enum defaulted to index 0 (Aggression), which is rarely the intended target — Phase 3 finding #5 surfaced this against a real consumer scenario. Scope-locked to ActorValue for v2.8.0; other Condition-function parameter slots (FormLink-typed args on `GetIsID` / `GetInFaction` / `GetInCell` / etc.) remain v2.9 candidates.

### Fixed — bridge

- **`perk_quest_adapter_subclass`** — `attach_scripts` on PERK and QUST records without an existing VMAD now succeeds. Mutagen 0.53.1's three concrete adapters (`VirtualMachineAdapter`, `PerkAdapter`, `QuestAdapter`) are sibling classes under the abstract base `AVirtualMachineAdapter`, not a base→subclass hierarchy. Pre-v2.8.0 the bridge's `ApplyAttachScripts` typed its local as `VirtualMachineAdapter` and constructed `new VirtualMachineAdapter()` for the auto-create path — both choices fail on PERK (whose `VirtualMachineAdapter` property is typed `PerkAdapter`) and QUST (`QuestAdapter`). Phase 2 reproduced the failure deterministically against vanilla Skyrim.esm. v2.8.0 types the local as the abstract base `AVirtualMachineAdapter` and constructs via `Activator.CreateInstance(vmadProp.PropertyType)` — PERK gets a `PerkAdapter`, QUST gets a `QuestAdapter`, every other type's property declares the base concrete `VirtualMachineAdapter` so the call reduces to the prior path. Coverage-smoke tests 158 (`4.c.06-rgr`) and 159 (`4.c.07-rgr`) lift Phase 2's race-probe Batch 7 PERK/QUST probes into the harness as positive regression cells (readback asserts the runtime adapter type matches the expected subclass).

### Changed — bridge

- **Helper-throw → Tier D unification on `add_conditions` / `remove_conditions`** (Phase 2 finding 4.c.01). Pre-v2.8.0 `ApplyAddConditions` and `ApplyRemoveConditions` threw `"Record type X does not support conditions"` when the target record lacked a `Conditions` property — bypassing Tier D's `unmatched_operators` coverage check and producing a structurally inconsistent error response compared to every other unsupported (operator, record-type) combo. v2.8.0 changes both helpers to return `int?`: `null` signals "no Conditions property", the call site skips writing the corresponding mods key, and Tier D's existing coverage check fires with uniform `unmatched_operators: ["add_conditions"]` / `["remove_conditions"]` shape. Coverage-smoke tests 3 (`remove_conditions on ARMO`) and 157 (`4.c.01-carry`, `add_conditions on QUST`) updated to assert the unified Tier D shape (these tests' assertions documented v2.7.1 behavior; v2.8.0 changes that documented behavior).

### Changed — schema description

- **`attach_scripts` supported-records list corrected** (Phase 4 items 1+2+3). `tools_patching.py:104` v2.7.1 wording listed Outfit and Spell as VMAD-supported types. Phase 4's three-stream evidence verification proved both are Mutagen 0.53.1 schema gaps: (1) typeof reflection finds no `VirtualMachineAdapter` property on the concrete class, on any interface, or in the full base-class chain; (2) full public-instance property dump finds zero VMAD-shaped properties (Outfit: 11 props, Spell: 26 props, case-sensitive PascalCase match against `Vmad`/`VMad`/`VMAD`/`Adapter` plus type-assignability check to `VirtualMachineAdapter`); (3) byte-scan of vanilla Skyrim.esm's SPEL and OTFT GRUP byte ranges finds zero `VMAD` subrecord signatures (Bethesda's own data has no precedent for VMAD on these record types). Outfit and Spell removed from the supported-records list. Cross-check probe over the remaining 11 types ({NPC, Quest, Armor, Weapon, Container, Door, Activator, Furniture, Light, MagicEffect, Perk}) confirmed every one exposes `VirtualMachineAdapter` via reflection (declared types: 9 × base `VirtualMachineAdapter`, Quest as `QuestAdapter`, Perk as `PerkAdapter`). Coverage-smoke 1.r.40 and 1.r.47 retain SKIP with the strengthened three-stream Case (A) reason; KNOWN_ISSUES.md captures the limitation.

### Documentation

- **Coverage-smoke matrix-cell labels** for tests 7-18 (Phase 4 item 7). The original Phase 1 add_keywords/remove_keywords cells (RACE/FURN/ACTI/LCTN/SPEL/MGEF) were unlabeled; Phase 2 introduced MATRIX cell IDs in helper labels for tests 31+. v2.8.0 backfills cell IDs for tests 7-18 (`1.A.01`/`1.A.07`/`1.A.10`/`1.A.13`/`1.A.16`/`1.A.19` for adds; `1.A.02`/`1.A.08`/`1.A.11`/`1.A.14`/`1.A.17`/`1.A.20` for removes). Phase 3 finding #1 had noted the pre-correction MATRIX cell 1.A.01 labeled `Skyrim.esm:08F95E` as VendorItemFood, but that FormID resolves to an audio category record; the harness was always correct (uses `FreshKwFor` predicate to pick a real KYWD), only the matrix label was wrong — corrected in MATRIX.md's Phase 3 corrections section.
...
- **Other Condition-function parameter slots.** v2.8.0 wired `actor_value`; FormLink-typed args on `GetIsID` / `GetInFaction` / `GetInCell` etc. remain unfilled (would require per-function dispatch + per-parameter-type routing — meaningfully more code than a single `Enum.Parse`).

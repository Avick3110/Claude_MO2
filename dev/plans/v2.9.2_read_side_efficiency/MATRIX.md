...
- [x] **4.dsl.06 missing-master synthetic fixture** — Phase 2 deferred to v2.9.x. Vanilla Skyrim.esm has no naturally-occurring missing-master FormLinks; building a synthetic in-memory plugin via Mutagen is heavyweight (round-trip-write fixture pattern from v2.7.1) and the wrapper-form null-safety contract is already exercised by 4.dsl.05 (always-null shape) + the bridge code's explicit handling in `ExpandFormLinkValue` (returns uniform `{formid, EditorID: null, expanded: null, error}` shape when `linkedRecord == null`). Cell 4.dsl.06 registers SKIP-with-reason in coverage-smoke. v2.9.x candidate when a real consumer surfaces a missing-master probe need.

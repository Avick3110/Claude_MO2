...
| T12 | .NET hard-block — **first-time verification for this change, deferred from P1 per Aaron's dev-machine constraint** (dev box has .NET 8; renaming `%PROGRAMFILES%\dotnet\` would break live MO2). P5 must provision a .NET-8-absent environment (VM, fresh Windows account, or equivalent) before declaring P1's `InitializeSetup` rewrite verified. | Installer aborts; no JSON or binary writes | n/a |

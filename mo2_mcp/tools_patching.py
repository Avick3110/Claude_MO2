                ...
                "records": {
                    ...
                    "items": {
                        ...
                        "properties": {
                            ...
                            "set_fields": {"type": "object", "description": "Field name->value pairs. Aliases: Health/Magicka/Stamina (NPC), ArmorRating/Value/Weight (ARMO), Damage/Speed/Reach (WEAP), BaseHealth/BaseMagicka/BaseStamina/HealthRegen/MagickaRegen/StaminaRegen (RACE). Also accepts Mutagen property paths like 'Configuration.HealthOffset'. Dict-typed fields support bracket syntax: 'Starting[Health]: 100' on RACE; works for any Mutagen IDictionary<,> property. Whole-dict assignment via JSON object: 'Starting: {Health: 100, Magicka: 200}' (merge semantics — only specified keys touched). Effects array (SPEL/ALCH/ENCH/SCRL/INGR) accepts JSON Array form for whole-list replacement: 'Effects: [{BaseEffect: \"Plugin:01ABCD\", Data: {Magnitude: 50, Area: 0, Duration: 30}, Conditions: [{function, operator, value, ...}]}]'. Replace semantics — the source list is cleared and the JSON-supplied entries written. Per-effect Conditions take the same shape as the add_conditions operator. PERK records also accept set_fields:{Effects:[...]} (v2.9.3) — same JSON Array replace-semantics, but each entry MUST carry an explicit 'type' discriminator naming the concrete Mutagen.Bethesda.Skyrim APerkEffect leaf class. Twelve valid 'type' values: PerkAbilityEffect, PerkEntryPointAbsoluteValue, PerkEntryPointAddActivateChoice, PerkEntryPointAddLeveledItem, PerkEntryPointAddRangeToValue, PerkEntryPointModifyActorValue, PerkEntryPointModifyValue, PerkEntryPointModifyValues, PerkEntryPointSelectSpell, PerkEntryPointSelectText, PerkEntryPointSetText, PerkQuestEffect. Per-leaf shape: PerkEntryPointModifyValue/PEPMs/PEPMA take EntryPoint (91-member enum like 'ModSpellMagnitude', 'ModBowDamage', 'Activate'), Modification (per-class enum: PEPM/PEPMs use {Set, Add, Multiply}; PEPMA uses its own enum), Value (Single, Nullable on PEPM/PEPMs, plain on PEPMA), with PEPMs adding Value2 and PEPMA adding ActorValue (e.g. 'Destruction'); PerkEntryPointSelectSpell takes EntryPoint + Spell (FormID); PerkEntryPointAddActivateChoice takes EntryPoint + Spell (nullable FormID); PerkEntryPointAddLeveledItem takes EntryPoint + Item (LVLI FormID); PerkEntryPointAddRangeToValue takes EntryPoint + From/To (Singles); PerkEntryPointAbsoluteValue takes EntryPoint + Negative (bool); PerkEntryPointSelectText takes EntryPoint + Text (plain string); PerkEntryPointSetText takes EntryPoint + Text (TranslatedString — plain JSON string accepted, written as English-language entry); PerkAbilityEffect takes Ability (SPEL FormID); PerkQuestEffect takes Quest (QUST FormID) + Stage (Byte 0–255). PerkQuestEffect.Unknown is an opaque MemorySlice<Byte> blob and is rejected at write-time — omit it; the source record's value carries through. Per-effect Conditions on the APerkEffect base use a TWO-LEVEL nesting: outer 'Conditions' is a list of PerkCondition wrappers each carrying RunOnTabIndex (int) plus an inner 'Conditions' list whose entries take the same shape as the add_conditions operator (function/operator/value/global/run_on/parameters/etc.) — including v2.9.0's per-function parameter dispatcher (e.g. inner condition with function='HasPerk' + parameters:{Perk:'<formid>'} works inside Effects[i].Conditions[j].Conditions[k] exactly as it does at the top-level add_conditions operator)."},
                            ...
                            "add_conditions": {"type": "array", "items": {"type": "object", "properties": {"function": {"type": "string", "description": "Mutagen condition function name (e.g. 'GetLevel', 'GetActorValue', 'GetIsID', 'HasPerk', 'GetGlobalValue')"}, "operator": {"type": "string", "description": "Comparison operator: ==, !=, <, <=, >, >=, or Mutagen enum name"}, "value": {"type": "number", "description": "ConditionFloat literal — compare against this value"}, "global": {"type": "string", "description": "ConditionGlobal FormID — compare against this Global (alternative to 'value'). Use with GetGlobalValue functions."}, "run_on": {"type": "string", "description": "Subject / Target / Reference / CombatTarget / LinkedReference / QuestAlias / PackageData / EventData (default: Subject)"}, "actor_value": {"type": "string", "description": "v2.8 back-compat sugar for parameters: {ActorValue: ...}. ActorValue enum name (e.g. 'Health', 'Magicka', 'Stamina', 'OneHanded', 'Sneak'). Used by GetActorValue / GetBaseActorValue / GetActorValuePercent. Either this OR parameters.ActorValue may be supplied; supplying both surfaces an unambiguous-DSL error."}, "parameters": {"type": "object", "description": "v2.9 — generic Condition-function parameter slot map. Keys are Mutagen reflection property names on the function's ConditionData class (e.g. parameters: {Object: '<formid>'} for GetIsID, parameters: {Faction: '<formid>'} for GetInFaction, parameters: {Perk: '<formid>'} for HasPerk, parameters: {ActorValue: 'Health'} for GetActorValue, parameters: {Quest: '<formid>', Stage: 50} for GetStageDone, parameters: {ReferenceAliasIndex: 0} for GetIsAliasRef). Values are JSON-typed per the slot's runtime type — string FormID for IFormLinkOrIndex<T> + IFormLink<T>, string enum-member-name for enum (case-insensitive parse; numeric input rejected), number for Int32/Single (string input rejected), bool for Boolean (Boolean unimplemented in v2.9.0 — design-only, no in-scope consumer). v2.9.0 covers 199 functions across five dispatcher branches (5 of 6 PLAN-named branches landed; Boolean is the single design-vs-implementation gap): 113 single-IFormLinkOrIndex<T> P2A (GetIsID/Object, GetInFaction/Faction, GetInCell/Cell, HasMagicEffect/MagicEffect, HasPerk/Perk, HasSpell/Spell, GetIsRace/Race, HasKeyword/Keyword, IsInList/FormList, GetItemCount/ItemOrList, etc.) + 6 sub-A IFormLink<T> P2A (GetVATSValue* family — Value slot) + 41 single-Enum P2B (ActorValue family + Sex/Axis/CastSource/FormType/Alignment/etc. — 18 distinct enum types incl. nested types like GetVATSValueActionConditionData+Action and Projectile+TypeEnum) + 28 MultiSlot P2C (GetStageDone/Quest+Stage canonical, GetEventData/Function+Member+Record 3-slot mixed-shape canary, GetCrime/GetFactionCombatReaction/GetRefTypeAliveCount/etc. dual-FLI, GetKeywordDataForAlias/IsSceneActionComplete/LocAliasHasKeyword/etc. FLI+Int32, GetRelativeAngle/IsCurrentSpell/SpellHasKeyword Enum+FLI, GetWithinDistance Single+FLI; the special 'Unknown' generic-fallback type is in-scope at the bridge level but coverage-smoke registers it as SKIP-with-reason since Mutagen reclassifies it on binary round-trip via the function code) + 11 PrimitiveOnly P2D (alias-index lookups GetIsAliasRef/GetInCurrentLocAlias/GetIsEditorLocAlias/GetLocationAliasCleared/IsLocAliasLoaded, package-data accessors GetNumericPackageData/GetWithinPackageLocation/IsNullPackageData, GetVATSValueUnknown 2-slot Int32, GetPlayerControlsDisabled 2-slot Int32, IsLimbGone — all single- or dual-slot Int32). Multi-slot composition routes through BuildCondition's foreach over ce.Parameters — each (slotName, value) pair dispatches independently through whichever branch matches its prop type. The Int32 + Single primitive branches landed in P2C cover all primitive-typed slots in the v2.9.0 in-scope set; Boolean is the only PLAN-§A branch not implemented (zero in-scope consumers verified across the 199 dispatcher-wired functions; first v2.9.x consumer trigger lands the branch). The v2.8 actor_value field stays as back-compat sugar for parameters: {ActorValue: ...}; supplying both forms surfaces an unambiguous-DSL error. Functions outside the v2.9.0 in-scope set surface a clean per-record 'not yet wired' error when parameters is supplied; called without parameters they preserve v2.7.1+ behavior (structurally-valid but always-false). CTDA padding slots (any name containing 'Unused' — these are Mutagen's mirror of CTDA's 4-parameter binary format and are never set in practice) are explicitly rejected by the dispatcher's footgun-guard. Per-function slot signatures: see CONDITIONS_AUDIT.md."}, "or_flag": {"type": "boolean", "description": "OR with the next condition (default: AND)"}}, "required": ["function"]}, "description": "Conditions to add. Supported on records with a top-level Conditions property (MagicEffect, Perk, Package, IdleAnimation, INFO via response-level conditions, plus other single-Conditions carriers) AND on QUST via the v2.9.1 condition_target operator parameter (which selects DialogConditions or EventConditions). SPEL records take per-effect conditions on their MGEFs (use the MGEF FormID), or via set_fields: {Effects: [...]} with nested Conditions."},
                            "remove_conditions": {"type": "array", "items": {"type": "object", "properties": {"function": {"type": "string"}, "index": {"type": "integer"}}}, "description": "Conditions to remove by function name or index. Same supported records as add_conditions (single-Conditions carriers MagicEffect/Perk/Package/etc., plus QUST via v2.9.1 condition_target)."},
                            "condition_target": {"type": "string", "description": "v2.9.1 — selects which condition list to write to (add_conditions) or remove from (remove_conditions) on multi-condition-list carriers. Required on QUST records (which carry DialogConditions and EventConditions rather than a single Conditions list); missing condition_target on a QUST add_conditions/remove_conditions call surfaces a clean per-record error naming the available targets. Valid values: 'dialog' (→ DialogConditions), 'event' (→ EventConditions). Case-insensitive. Records with a single Conditions property (MagicEffect, Perk, Package, IdleAnimation, INFO, etc.) reject condition_target explicitly with a per-record error directing the caller to omit the parameter. Records with no condition list at all (e.g. ARMO) fall through to Tier D's uniform unmatched_operators error shape (existing v2.9.0 behavior preserved bit-identically). v2.9.1 in-scope record types: QUST only (Mutagen 0.53.1 schema probe found Quest is the sole multi-condition top-level record type). Nested *Conditions surfaces (IQuestAliasGetter.Conditions + IQuestLogEntryGetter.Conditions) deferred to v2.9.x — see KNOWN_ISSUES.md § Patching write surface."},
                            "attach_scripts": {"type": "array", "items": {"type": "object", "properties": {"name": {"type": "string"}, "properties": {"type": "array", "items": {"type": "object", "properties": {"name": {"type": "string"}, "type": {"type": "string", "enum": ["Object", "Int", "Float", "Bool", "String", "Alias"], "description": "Alias type expects value as {quest: 'Plugin:FormID', alias_id: int}"}, "value": {}}}}}, "required": ["name"]}, "description": "Scripts to attach with typed properties (VMAD). Supported on records whose Mutagen 0.53.1 schema exposes a VirtualMachineAdapter property: NPC, Quest, Armor, Weapon, Container, Door, Activator, Furniture, Light, MagicEffect, Perk. PERK and QUST use subclassed adapters (PerkAdapter, QuestAdapter); the bridge auto-creates the matching subclass when attaching to a record with no existing scripts. Outfit and Spell are NOT supported — their Mutagen schemas don't expose VMAD and Bethesda's vanilla Skyrim.esm SPEL/OTFT records don't carry VMAD subrecords (see KNOWN_ISSUES)."},
